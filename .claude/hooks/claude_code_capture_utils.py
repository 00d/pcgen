...
def get_ab_metadata(cwd):
    ...
    # Inject the lane's blinding alias as model_name, not the real model.
    # The expert can `cat` their own logs/model_a/*.jsonl after the session;
    # writing the real "claude-opus-4-7" string here used to defeat the
    # blinding. The real assignment is still tracked in manifest.assignments
    # for the server-side extractor.
    code_names = manifest.get("code_names", {})
    assignments = manifest.get("assignments", {})
    metadata["model_name"] = code_names.get(model_lane) or assignments.get(model_lane)
