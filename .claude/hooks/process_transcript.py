...
def read_and_process_raw_transcript(transcript_path, strip_model_names=True):
    ...
    for i, event in enumerate(raw_events):
        ...
        # Strip model identifiers from any event for blinding. Two surfaces:
        # 1. `event['message']['model']` — Claude Code's API-response capture
        # 2. `event['model']` — same string promoted to the event top-level
        #    by some captures (assistant_thinking, etc.) — older strip logic
        #    missed this and the real model name (e.g. "claude-opus-4-7")
        #    leaked into the cleaned JSONL even when message.model was
        #    deleted. Deep-copy once and prune both locations.
        if strip_model_names:
            message = event.get('message') or {}
            top_level_model_real = isinstance(event.get('model'), str) and event['model'].startswith('claude-')
            msg_model_real = isinstance(message.get('model'), str) and message.get('model', '').startswith('claude-')
            if top_level_model_real or msg_model_real:
                event = json.loads(json.dumps(event))  # deep copy before mutating
                if msg_model_real:
                    del event['message']['model']
                if top_level_model_real:
                    del event['model']
